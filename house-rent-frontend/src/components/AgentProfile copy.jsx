...
import { housesAPI, agentsAPI, reviewsAPI } from '../services/api'
                ...
                <div className="border-t pt-6">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{agent.stats?.total_rentals ?? agent.stats?.totalRentals ?? agent.agent_stats?.total_rentals ?? '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Response Time:</span>
                      <span className="font-medium">{agent.stats?.average_response_time ?? agent.stats?.averageResponseTime ?? agent.agent_stats?.average_response_time ?? '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Client Satisfaction:</span>
                      <span className="font-medium">{agent.stats?.client_satisfaction ?? agent.stats?.clientSatisfaction ?? agent.agent_stats?.client_satisfaction ?? '-'}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Repeat Clients:</span>
                      <span className="font-medium">{agent.stats?.repeat_clients ?? agent.stats?.repeatClients ?? agent.agent_stats?.repeat_clients ?? '-'}</span>
                    </div>
                  </div>
                </div>
              </CardContent>
                          ...
                          <div className="p-4">
                            ...
                            <div className="flex items-center justify-between mb-3">
                              <span className="text-xl font-bold text-blue-600">
                                {property.price !== undefined && property.price !== null ? `$${property.price.toLocaleString()}/mo` : '-'}
                              </span>
                            </div>
